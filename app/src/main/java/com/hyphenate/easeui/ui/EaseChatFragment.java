...
//    private boolean isRecording = false;
    ...
    public void onBeginDownloadForwad(BeginDownloadForwad beginDownloadForwad ) {
       ...
       /* conversation.removeMessage(beginDownloadForwad.getMsgId() + "");
        ...
        //message.setMsgTime(fileData.getTimestamp() * 1000);
        KLog.i(msgId+"：onBeginDownloadForwad:"   +message.getMsgTime() +"&&" +fileData.getTimestamp());
                    ...
                    if (eMMessage != null && LogIdIdResult != null) {
                        ...
                        if (eMMessage.getType().equals(EMMessage.Type.IMAGE))
                        {
                            ...
                            if(localMedia != null)
                            {
                                ...
                                localMedia.setSortIndex(Integer.valueOf(LogIdIdResult));
                                previewImages.add(localMedia);
                                ImagesObservable.getInstance().saveLocalMedia(previewImages,"chat");
...
//                        String fileName = leftName + "_" + ((int) (System.currentTimeMillis() / 1000)) + typeName;
    ...
    protected void sendFileMessageaaa(String filePath) {
        new Thread(new Runnable() {
            public void run() {
...
//                                FileUtil.drawableToFile(AppConfig.instance,R.mipmap.kong,fileOrginName,5);
    ...
    public void receiveTxtMessage(JPushMsgRsp jPushMsgRsp) {
        ...
        String userSn = SpUtil.INSTANCE.getString(getActivity(), ConstantValue.INSTANCE.getUserSnSp(), "");
        if (Message.getSender() == 0) {
            SpUtil.INSTANCE.putString(AppConfig.instance, ConstantValue.INSTANCE.getMessage() + userSn + "_" + jPushMsgRsp.getParams().getFromId(), baseDataJson);
        } else {
            SpUtil.INSTANCE.putString(AppConfig.instance, ConstantValue.INSTANCE.getMessage() + userSn + "_" + jPushMsgRsp.getParams().getFromId(), baseDataJson);
        }
        sendMessageTo(message);
    ...
    public void receiveTxtMessageV3(JPushMsgRsp jPushMsgRsp) {
        ...
        String userSn = SpUtil.INSTANCE.getString(getActivity(), ConstantValue.INSTANCE.getUserSnSp(), "");
        if (Message.getSender() == 0) {
            SpUtil.INSTANCE.putString(AppConfig.instance, ConstantValue.INSTANCE.getMessage() + userSn + "_" + jPushMsgRsp.getParams().getFrom(), baseDataJson);
        } else {
            SpUtil.INSTANCE.putString(AppConfig.instance, ConstantValue.INSTANCE.getMessage() + userSn + "_" + jPushMsgRsp.getParams().getFrom(), baseDataJson);
        }
        sendMessageTo(message);
     ...
     * @param toId
     */
    public void receiveFileMessage(String fileName, String msgId, String fromId, String toId, int FileType, String fileInfo,int timeStamp) {
        String files_dir = "";
        EMMessage message = null;
        KLog.i("收到了文件消息，。。。");
        switch (FileType) {
            case 1:
                files_dir = PathUtils.getInstance().getFilePath() + "/" + fileName;
                message = EMMessage.createImageSendMessage(files_dir, true, toChatUserId);
...
//                }, 500);
    ...
    protected void selectPicFromCamera() {
        ...
        try {
            ...
            Uri uri= EaseCompat.getUriForFile(getContext(), cameraFile);
